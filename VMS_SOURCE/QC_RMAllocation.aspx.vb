...
Partial Class QC_RMAllocation
    Inherits System.Web.UI.Page
    Dim userInfo As VMSUserEntity = New VMSUserEntity()
    Private ReadOnly ConnStr As String = "YOUR_CONNECTION_STRING_HERE"
...
#End Region
    ...
    Protected Sub btnUpload_Click(sender As Object, e As EventArgs)
        ...
        Dim dt As DataTable = ImportAllocationExcel(filePath)

        If dt.Rows.Count = 0 Then
            ' lblMsg.Text = "No valid data found"
    ...
    Public Shared Function ImportAllocationExcel(filePath As String) As DataTable
        ...
        dt.Columns.Add("Alloted Qty", GetType(String))
        ...
        Using fs As New FileStream(filePath, FileMode.Open, FileAccess.Read)
            ...
            ' READ MAIN SHEET
            ' =========================
            Dim sheet As ISheet = workbook.GetSheet("RMAllocation")
            ...
            For rowIndex As Integer = 1 To sheet.LastRowNum
                ...
                Dim allotedqty As String = GetCellValue(row.GetCell(3))
                Dim price As String = GetCellValue(row.GetCell(4))
    ...
    Private Sub SubmitBulkData(ByVal dt As DataTable)
        ...
        Try
            ...
            RecordInserted = obj.InsertAllocationDetails(ddlQuarter.SelectedValue, userInfo.userIDEntity, dt)
    ...
    Private Function ReadExcelUsingNPOI(filePath As String) As DataTable
        ...
        Using fs As New FileStream(filePath, FileMode.Open, FileAccess.Read)
            ...
            If header Is Nothing OrElse
               header.GetCell(0).ToString() <> "Name" OrElse
               header.GetCell(1).ToString() <> "Score" Then
    ...
    Private Sub ExportToExcel(ds As DataSet)
        ...
        Dim sheet As HSSFSheet = workbook.CreateSheet("RMAllocation")
        ...
        headerRow.CreateCell(3).SetCellValue("Allocated Qty")
        headerRow.CreateCell(4).SetCellValue("Price")
        ' headerRow.CreateCell(5).SetCellValue("Supplier_ID")
        ...
        '==============================
        ' ALLOCATED QTY VALIDATION (Column 3 → D)
        '==============================
        Dim allocQtyRange As New CellRangeAddressList(1, 100, 3, 3)
        Dim allocQtyConstraint As DVConstraint =
            DVConstraint.CreateNumericConstraint(ValidationType.DECIMAL, OperatorType.GREATER_THAN, "0", Nothing)
        Dim allocQtyValidation = dvHelper.CreateValidation(allocQtyConstraint, allocQtyRange)
        allocQtyValidation.ErrorStyle = ERRORSTYLE.STOP
        allocQtyValidation.CreateErrorBox("Invalid Allocated Qty", "Allocated Qty must be a numeric value greater than 0. Text is not allowed.")
        allocQtyValidation.ShowErrorBox = True
        allocQtyValidation.EmptyCellAllowed = False
        sheet.AddValidationData(allocQtyValidation)

        '==============================
        ' PRICE VALIDATION (Column 4 → E)
        '==============================
        Dim priceRange As New CellRangeAddressList(1, 100, 4, 4)
        Dim priceConstraint As DVConstraint =
            DVConstraint.CreateNumericConstraint(ValidationType.DECIMAL, OperatorType.GREATER_THAN, "0", Nothing)
        Dim priceValidation = dvHelper.CreateValidation(priceConstraint, priceRange)
        priceValidation.ErrorStyle = ERRORSTYLE.STOP
        priceValidation.CreateErrorBox("Invalid Price", "Price must be a numeric value greater than 0. Text is not allowed.")
        priceValidation.ShowErrorBox = True
        priceValidation.EmptyCellAllowed = False
