...
#Region "Bind Grid"
    Private Sub bindGrid()
        ...
        If (ddlProduct.SelectedValue.ToString()) = "" Then
            ...
            Exit Sub
        End If
        If (ddlSku.SelectedValue.ToString()) = "" Then
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "alert", "alert('Please select a sku.');", True)
            Exit Sub
        End If
        ...
        If (Not (LovDetailsList Is Nothing) AndAlso LovDetailsList.Tables.Count > 0) Then
            If (Not (LovDetailsList.Tables(0) Is Nothing) AndAlso LovDetailsList.Tables(0).Rows.Count > 0 AndAlso Not (LovDetailsList.Tables(1) Is Nothing) AndAlso LovDetailsList.Tables(1).Rows.Count > 0) Then
                ...
                '  gvExteriorTestList.Rows(0).Cells("Ref Value"). = skupacksize
...
#End Region
    ...
    Protected Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnSubmit.Click
        ...
        dt.Columns.Add("ref_value", GetType(String))
        ...
        For Each row As GridViewRow In gvExteriorTestList.Rows
            ...
            Dim ddlResultValue As DropDownList = CType(row.FindControl("ddlResultValue"), DropDownList)
            Dim txtResultValue As TextBox = CType(row.FindControl("txtResultValue"), TextBox)
            Dim hdnSkuVale As HiddenField = CType(row.FindControl("hdnSkuVale"), HiddenField)
            Dim resultValue As String = String.Empty
            'Dim packsize() As String = hdnpackSize.Value.Split("k")
            'Dim skupacksize As String = packsize(1)
            Dim refValue As String = String.Empty
            ...
            Else
                resultValue = txtResultValue.Text.ToString()
            End If
            If Val(hdnTestId.Value = 139) Then
                refValue = hdnSkuVale.Value
            Else
                refValue = 0
            End If
            If Val(hdnTestId.Value) > 0 Then
        ...
        If dt.Rows.Count > 0 Then
            ...
            Try
                ...
                RecordInserted = obj.DeviationFGQualityInsertUpdate(quarter, vendorId, brandId, userInfo.userIDEntity, check, ddlProduct.SelectedValue, ddlSku.SelectedValue, txtBatchNo.Text, dt, sqlConn, sqlTrans)
                If (RecordInserted(0) > 0) Then
                    'sqlTrans.Rollback()
                    sqlTrans.Commit()
                    bindGrid()
                    If Val(hdnId.Value) > 0 Then
                    ...
                    End If
                ElseIf (RecordInserted(0) = -3) Then
                    Dim msg As String = RecordInserted(1)
                    ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "alert", "alert('" & msg & "');", True)
                Else
                    sqlTrans.Rollback()
