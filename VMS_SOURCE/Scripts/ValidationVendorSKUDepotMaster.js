...
function ValidateUpdate(NewVendor, lblSKU, lblDepot, lblCurrentVendor, lblNewVendorName, lblValidationMessage, btnSubmit) {
    firstErrorControl = "";
    errMsg = "";
    debugger;
    ...
    if (firstErrorControl != "") {
        ...
        document.getElementById(lblValidationMessage).innerHTML = "";
        if (confirm("SKU ( " + SKU + " ) against Depot ( " + Depot + " ) will be delinked from Current Vendor ( " + CurrentVendor + " ) and linked to New Vendor (" + newvendor + "). !! \r\n Are u sure to update?")) {
            return true;
        }
        else {
            return false;
...
function ValidateInsert(Vendor, Depot, TslFactor,SKU,SKUDESC, lblValidationMessage, btnSubmit) {
    firstErrorControl = "";
    errMsg = "";
    debugger;
    ValidateRequired(Vendor, "Please select valid vendor source.");
    ValidateRequired(Depot, "Please select valid depot.");
    if (ValidateRequired(TslFactor, "Please enter valid TSL factor."))
    {
      ValidateDecimal(TslFactor, "Please Enter valid TSL factor.");
    }
