...
Partial Class FactoryWiseDespatch
    ...
    Private Sub PopulateUnit()
        ...
        UnitSet = UnitDespatch.GetFactryList(userInfo.userIDEntity)
        If (Not (UnitSet Is Nothing) AndAlso UnitSet.Tables.Count > 0 AndAlso Not (UnitSet.Tables(0) Is Nothing) AndAlso UnitSet.Tables(0).Rows.Count > 0) Then
            ...
            ddlUnit.DataTextField = "factory_name"
            ddlUnit.DataValueField = "factory_code"
            ddlUnit.DataBind()
...
#End Region
    ...
    Protected Sub btndownload_Click(sender As Object, e As EventArgs)
        Try
            ...
            ExcelSet = reportObj.GetFactoryWiseDespatchReport(ddlProcessYr.SelectedValue, ddlProcessMnth.SelectedValue, userInfo.userIDEntity, ddlproductcat.SelectedValue, ddlproductcode.SelectedValue, ddlRegion.SelectedValue, ddlDepot.SelectedValue, ddlUnit.SelectedValue)
    ...
    Private Sub ExportToExcelSheet(ByVal ds As DataSet)
        ...
        Dim fs As FileStream = New FileStream(Server.MapPath(Request.ApplicationPath) & "\Templates\Factory_Wise_Unit_Despatch_Report_Template.xlsx", FileMode.Open, FileAccess.Read)
        ...
        Dim vmsSKUVol As Decimal = 0
        Dim ddfactoryskuvol As Decimal = 0
        Dim TotalDD As Decimal = 0
        Dim TotalDespatch As Decimal = 0

        If dt.Rows.Count > 0 Then
            RowsIndex = 2
            row = CType(sheet1.GetRow(0), XSSFRow)
            cell = CType(row.GetCell(0), XSSFCell)
            cell.SetCellValue("Source Factory Wise Report - " & ddlProcessMnth.SelectedItem.Text & ddlProcessYr.SelectedValue)
            cell = CType(row.GetCell(0), XSSFCell)
            ...
            For i = 0 To dt.Rows.Count - 1
                ...
                cell = CType(row.CreateCell(Math.Min(System.Threading.Interlocked.Increment(CellIndex), CellIndex - 1)), XSSFCell)
                cell.SetCellValue(Convert.ToDecimal(dt.Rows(i)("vms_SKU_Vol")))
                cell.CellStyle = styleRight

                vmsSKUVol = vmsSKUVol + Convert.ToDecimal(dt.Rows(i)("vms_SKU_Vol"))

                cell = CType(row.CreateCell(Math.Min(System.Threading.Interlocked.Increment(CellIndex), CellIndex - 1)), XSSFCell)
                cell.SetCellValue(Convert.ToDecimal(dt.Rows(i)("dd_factory_sku_vol")))
                cell.CellStyle = styleRight

                ddfactoryskuvol = ddfactoryskuvol + Convert.ToDecimal(dt.Rows(i)("dd_factory_sku_vol"))

                cell = CType(row.CreateCell(Math.Min(System.Threading.Interlocked.Increment(CellIndex), CellIndex - 1)), XSSFCell)
                cell.SetCellValue(Convert.ToDecimal(dt.Rows(i)("total_despatch")))
                cell.CellStyle = styleRight

                TotalDespatch = TotalDespatch + Convert.ToDecimal(dt.Rows(i)("total_despatch"))

                cell = CType(row.CreateCell(Math.Min(System.Threading.Interlocked.Increment(CellIndex), CellIndex - 1)), XSSFCell)
                cell.SetCellValue(Convert.ToDecimal(dt.Rows(i)("Total_DD")))
                cell.CellStyle = styleRight

                TotalDD = TotalDD + Convert.ToDecimal(dt.Rows(i)("Total_DD"))
            ...
            cell = CType(row.CreateCell(Math.Min(System.Threading.Interlocked.Increment(CellIndex), CellIndex - 1)), XSSFCell)
            cell.SetCellValue(vmsSKUVol)
            cell.CellStyle = styleTotal

            cell = CType(row.CreateCell(Math.Min(System.Threading.Interlocked.Increment(CellIndex), CellIndex - 1)), XSSFCell)
            cell.SetCellValue(ddfactoryskuvol)
            cell.CellStyle = styleTotal

            cell = CType(row.CreateCell(Math.Min(System.Threading.Interlocked.Increment(CellIndex), CellIndex - 1)), XSSFCell)
            cell.SetCellValue(TotalDespatch)
            cell.CellStyle = styleTotal

            Dim PercentageDD As Decimal = 0
            PercentageDD = Math.Round((ddfactoryskuvol / (vmsSKUVol + ddfactoryskuvol) * 100), 2)
        ...
        End If
        Dim file_name As String = "Factory_Wise_Unit_Despatch_Report" + DateString + ".xlsx"
